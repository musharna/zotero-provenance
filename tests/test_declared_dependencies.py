...
def test_every_third_party_import_is_a_declared_dependency() -> None:
    ...
    declared = {
        m.group(1).lower() for m in re.finditer(r'^\s*"([A-Za-z0-9_.-]+)', block.group(1), re.M)
    }
    local = {p.stem for p in SCRIPTS.glob("*.py")} | {
        p.name for p in SCRIPTS.iterdir() if p.is_dir()
    }
    ...
    missing = sorted(
        m for m in third_party if DIST.get(m, m).replace("_", "-").lower() not in declared
    )
