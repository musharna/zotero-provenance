...
def test_append_failure_writes_jsonl(tmp_path: Path):
    ...
    append_failure(queue, {"url": "https://example.com/x", "error": "503"})
    append_failure(queue, {"url": "https://example.com/y", "error": "504"})
    lines = queue.read_text().splitlines()
    assert len(lines) == 2
    assert json.loads(lines[0])["url"] == "https://example.com/x"


def test_drain_calls_handler_and_removes(tmp_path: Path):
    queue = tmp_path / "retry.jsonl"
    append_failure(queue, {"url": "https://example.com/x", "error": "503"})
    append_failure(queue, {"url": "https://example.com/y", "error": "503"})
    ...
    assert n == 2
    assert sorted(drained) == ["https://example.com/x", "https://example.com/y"]
...
def test_drain_keeps_malformed_lines_and_continues(tmp_path: Path):
    queue = tmp_path / "retry.jsonl"
    queue.write_text(
        'not valid json\n{"url": "https://example.com/x", "error": "503"}\n'
    )
    ...
    n = drain_queue(queue, handler)
    assert n == 1
    assert drained == ["https://example.com/x"]
...
def test_drain_keeps_failed_entries(tmp_path: Path):
    ...
    assert json.loads(remaining[0])["url"] == "https://example.com/y"
