...
def test_size_is_no_longer_an_outcome_at_all() -> None:
    """It never was a failure OF THE SOURCE -- the host served the document
    perfectly -- and it is no longer a failure of ours either: an oversized read
    now yields a digest over a stated prefix, so there is nothing left to
    classify. The vocabulary is for things that stopped us, and size does not.

    Asserted over the module rather than against a name, so deleting the outcome
    and leaving a producer behind cannot pass."""
...
def test_a_failed_fetch_is_recorded_as_attempted(db: Path) -> None:
    ...
    _run(db, lambda url, max_bytes=0: (_ for _ in ()).throw(_status(403)))
...
def test_a_recorded_failure_is_not_retried_by_default(db: Path) -> None:
    ...
    _run(db, lambda url, max_bytes=0: (_ for _ in ()).throw(_status(404)))
