...
def test_lookup_url_returns_none_for_missing(tmp_db: Path):
    init_db(tmp_db)
    assert lookup_url(tmp_db, "https://example.com/foo") is None
...
def test_insert_then_lookup_roundtrip(tmp_db: Path):
    ...
    row = lookup_url(tmp_db, "https://example.com/foo")
...
def test_update_last_seen(tmp_db: Path):
    init_db(tmp_db)
    insert_url(tmp_db, "https://example.com/foo", "ABC123", date(2026, 5, 1))
    update_last_seen(tmp_db, "https://example.com/foo", date(2026, 5, 5))
    row = lookup_url(tmp_db, "https://example.com/foo")
    assert row is not None
    assert row["last_seen"] == "2026-05-05"
...
def test_insert_url_duplicate_silently_ignored(tmp_db: Path):
    init_db(tmp_db)
    insert_url(tmp_db, "https://example.com/dup", "KEY_A", date(2026, 5, 1))
    # Second insert with a different key must not raise
    insert_url(tmp_db, "https://example.com/dup", "KEY_B", date(2026, 5, 5))
    row = lookup_url(tmp_db, "https://example.com/dup")
    assert row is not None
