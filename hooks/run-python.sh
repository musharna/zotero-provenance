...
#
# This launcher carries the same trampoline as the capture hooks, and for a
# sharper reason. A session keeps whichever plugin root it resolved at its own
# start, and commands/triage.md invokes ${CLAUDE_PLUGIN_ROOT}/hooks/run-python.sh
# -- so a superseded session ran superseded code that MUTATES the library. The
# capture hooks can argue for availability over correctness, because dropping a
# citation is a real cost. A command a person typed has no such argument: if the
# authority cannot be resolved, refuse and say so.

set -uo pipefail

# What this script does when the trampoline declines. Kept OUT of the trampoline
# block so that block can stay byte-identical across all four copies. Unlike the
# capture hooks, this one is loud and non-zero: a person is waiting for output.
zp_tramp_refuse() {
	printf 'zotero-provenance: refusing to run from a superseded plugin root: %s\n' \
		"${1:-unknown}" >&2
	exit 3
}
...
source "$HOOK_DIR/lib.sh" || {
	printf 'zotero-provenance: lib.sh is missing or unreadable\n' >&2
	exit 3
}
zp_load_secrets
